...
public class Layout_1v1v1v1 extends Layout {
	...
	public void build() {
		...
		//Player 4 slots
		Player player4 = PlayerManager.players.get(2);
		
		xpos = Main.scrw-ch-gap;
		ypos = (Main.scrh/2)-(ch)-(cw*2);
		
		slot = new CardSlot(xpos+(ch/2)-(cw/2),ypos);
		player4.addDeckSlot(slot);
		addSlot(slot);
		slot.setCanTouch(false);
	...
	public void drawPlayerNames(Graphics g) {
		...
		pl = PlayerManager.players.get(2);
